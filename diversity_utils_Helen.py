...
#####################################################################
def calculate_sigmasquared(allele_counts_1, allele_counts_2, low = 0.0, high = 1.0):
    # A standard measure of linkage disequilibrium:
    #
    # sigma_squared= E[X]/E[Y], where X=(p_ab-pa*pb)^2 and Y=(pa*(1-pa)*pb*(1-pb))
    # rsquared=E[X/Y]
    # see McVean 2002 for more notes on the difference. 
    # low and high are boundaries for conditional allele frequency, default: full range
    ...
    # check if any freqs >0.5, if so, fold:
    folded_pooled_freqs_1=numpy.where(marginal_pooled_freqs_1 > 0.5, 1-marginal_pooled_freqs_1, marginal_pooled_freqs_1)
    folded_pooled_freqs_2=numpy.where(marginal_pooled_freqs_2 > 0.5, 1-marginal_pooled_freqs_2, marginal_pooled_freqs_2) 

    # condition on designated allele frequency
    condition = (folded_pooled_freqs_1 > low)*(folded_pooled_freqs_1 < high)*(folded_pooled_freqs_2 > low)*(folded_pooled_freqs_2 < high)
       
    # (p_ab-p_a*p_b)^2
    rsquared_numerators = numpy.square(joint_pooled_freqs-marginal_pooled_freqs_1*marginal_pooled_freqs_2)*condition
    
    # (p_a*(1-p_a)*pb*(1-p_b))
    rsquared_denominators = marginal_pooled_freqs_1*(1-marginal_pooled_freqs_1)*marginal_pooled_freqs_2*(1-marginal_pooled_freqs_2)*condition
...
#####################################################################
def calculate_unbiased_sigmasquared(allele_counts_1, allele_counts_2, low = 0.0, high = 1.0):
    ...
    allele_frequency_1 = (n11s + n10s) / ns
    allele_frequency_2 = (n11s + n01s) / ns

    condition = (allele_frequency_1 > low)*(allele_frequency_1 < high)*(allele_frequency_2 > low)*(allele_frequency_2 > high)
    
    # rsquared_numerators = rsquared_numerators*(ns>3.5)*1.0/(ns*(ns-1)*(ns-2)*(ns-3)+10*(ns<3.5)) #orginal code

    rsquared_numerators = rsquared_numerators*(ns>3.5)*condition*1.0/(ns*(ns-1)*(ns-2)*(ns-3)+10*(ns<3.5))
    ...
    rsquared_denominators = rsquared_denominators*(ns>3.5)*condition*1.0/(ns*(ns-1)*(ns-2)*(ns-3)+10*(ns<3.5))
