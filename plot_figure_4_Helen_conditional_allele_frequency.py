...
import calculate_linkage_disequilibria_Helen as calculate_linkage_disequilibria
...
focal_speciess = ['Prevotella_copri_61740']
# focal_speciess = ['Bacteroides_uniformis_57318']
...
for focal_species_idx in xrange(0,len(focal_speciess)):
    ...
    focal_example_axis.set_xlim([2,4e03])
...
for species_idx in xrange(0,num_passed_species):
    ...
    all_distances, all_rsquared_numerators, all_rsquared_denominators, all_ns, all_intergene_distances, all_intergene_rsquared_numerators, all_intergene_rsquared_denominators, all_intergene_ns, all_control_rsquared_numerator, all_control_rsquared_denominator, all_control_n, all_pi = ld_map[('largest_clade','4D')]
    all_control_rsquared = all_control_rsquared_numerator/all_control_rsquared_denominator
             
    distances, rsquared_numerators, rsquared_denominators, ns, intergene_distances, intergene_rsquared_numerators, intergene_rsquared_denominators, intergene_ns, control_rsquared_numerator, control_rsquared_denominator, control_n, pi = ld_map[('largest_clade','1D')]
    ...
    if (species_name in focal_speciess) or (species_name in supplemental_focal_species):
        ...
        example_axis.loglog(all_distances, all_rsquareds,'-',linewidth=1,color='b',label='Synonymous')
        # example_axis.fill_between(numpy.array([3.3e03,1e04]),numpy.array([1e-02,1e-02]), numpy.array([1,1]),color='b',zorder=20)
        # example_axis.loglog([all_distances[-1],6e03], [all_rsquareds[-1], all_control_rsquared],':',color='b',zorder=21)
        ...
        example_axis.loglog(distances, rsquareds,'-',linewidth=1, color='r',label='Non-synonymous')
        # example_axis.loglog(early_distances, early_rsquareds,'o',color=color,markersize=2,markeredgewidth=0,alpha=0.5)
        
        # example_axis.loglog([distances[-1],6e03], [rsquareds[-1], control_rsquared],':',color='r',zorder=21)
